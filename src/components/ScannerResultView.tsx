...
export default function ScannerResultView({
  ...
  const getAuthedUserEmail = () => {
    if (typeof window === 'undefined') return '';
    return (
      localStorage.getItem('pawvalid_user_email') ||
      localStorage.getItem('petvia_user_email') ||
      ''
    ).toLowerCase().trim();
  };

  const handleOpenSaveModal = () => {
    const authed = getAuthedUserEmail();
    if (authed && !saveEmail) {
      setSaveEmail(authed);
    }
    setSaveError(null);
  ...
  const handleSaveTrip = async (e: React.FormEvent) => {
    ...
    const cleanInputEmail = saveEmail.trim().toLowerCase();
    if (!cleanInputEmail || !cleanInputEmail.includes('@') || !cleanInputEmail.includes('.')) {
      setSaveError('Please enter a valid email address to save your pet’s records.');
      return;
    }

    const effectivePetName = savePetName.trim() || activePetName || 'My Pet';
    const authedEmail = getAuthedUserEmail();
    const isAlreadyAuthed = authedEmail && authedEmail === cleanInputEmail;
    ...
    try {
      ...
      // If already authenticated with this email, direct access is safe and permitted
      if (isAlreadyAuthed) {
      ...
      // If NOT already authenticated, DO NOT grant immediate session access!
      // Require email ownership verification via signed magic token link
      ...
      if (!verifyRes.ok || !verifyData.success) {
        throw new Error(verifyData.error || 'Failed to dispatch verification link.');
      }

      setSentEmailAddress(cleanInputEmail);
      setDevMagicUrl(verifyData.devMagicUrl || null);
      setVerificationSent(true);
      setResendCooldown(30);
    } catch (err: any) {
      console.error('Error saving trip:', err);
      setSaveError(err.message || 'Failed to save trip. Please check your network and try again.');
    } finally {
  ...
  const handleResendVerification = async () => {
    if (!sentEmailAddress || resendCooldown > 0 || isResending) return;
    try {
      setIsResending(true);
      const verifyRes = await fetch('/api/auth/send-verification', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          petName: savePetName.trim() || activePetName,
          origin: displayOrigin,
          destination: displayDestination,
        }),
      });
      ...
      {isSaveModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0E2342]/70 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-white rounded-2xl p-6 sm:p-8 max-w-md w-full border border-zinc-200 shadow-2xl relative space-y-4">
            ...
            {verificationSent ? (
              /* Email Ownership Verification Sent View */
              <div className="space-y-4 text-center py-2">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-zinc-600 leading-relaxed max-w-sm mx-auto">
                    We sent a secure 1-click verification link to:
                  </p>
                  <div className="inline-block bg-zinc-100 border border-zinc-200 text-zinc-900 font-mono font-bold text-xs px-3 py-1.5 rounded-lg mt-1 break-all">
                    {sentEmailAddress}
                  </div>
                </div>
                ...
                <div className="bg-emerald-50/70 border border-emerald-200/80 rounded-xl p-3 text-left text-xs text-emerald-950 space-y-1.5">
                  ...
                  <p className="text-[11px] text-emerald-800 leading-relaxed">
                    To protect your pet’s health records and ensure that only you can access your travel vault, please confirm ownership of this email address before accessing the dashboard.
                  </p>
                </div>

                <div className="text-left text-xs space-y-1 text-zinc-600 bg-zinc-50 border border-zinc-200/60 rounded-xl p-3">
                  <p className="font-bold text-zinc-800 text-[11px] uppercase tracking-wide">Next Steps:</p>
                  <ol className="list-decimal pl-4 space-y-1 text-[11px] leading-relaxed">
                    <li>Open the verification email sent to <strong>{sentEmailAddress}</strong>.</li>
                    <li>Click <strong>&quot;Verify Email &amp; Open Dashboard&quot;</strong>.</li>
                    <li>You will be instantly signed in to your travel command center.</li>
                  </ol>
                </div>
                ...
                {devMagicUrl && (
                  <div className="p-2.5 bg-amber-50 border border-amber-200 rounded-xl text-left text-xs">
                    <div className="font-bold text-amber-900 text-[11px] flex items-center justify-between">
                      <span>🛠️ Local Dev Shortcut:</span>
                      <a
                        href={devMagicUrl}
                        className="underline text-amber-800 hover:text-amber-950 font-mono text-[10px]"
                      >
                ...
                <div className="pt-2 flex flex-col sm:flex-row items-center gap-2">
                  <button
                    ...
                    className="w-full sm:flex-1 py-2.5 rounded-lg border border-zinc-300 text-zinc-700 hover:bg-zinc-50 font-semibold text-xs transition-all cursor-pointer disabled:opacity-50"
                  >
                    {isResending
                      ? 'Sending...'
                      : resendCooldown > 0
                  ...
                  <button
                    ...
                    className="w-full sm:flex-1 py-2.5 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-zinc-800 font-semibold text-xs transition-all cursor-pointer"
                  >
                    Change Email
                  </button>
                </div>
              ...
              <>
                ...
                <form onSubmit={handleSaveTrip} className="space-y-3.5 pt-2 text-xs">
                  ...
                  <div>
                    ...
                    {getAuthedUserEmail() && getAuthedUserEmail() === saveEmail.trim().toLowerCase() ? (
                      <p className="text-[11px] text-emerald-600 mt-1.5 flex items-center gap-1.5 font-medium">
                        ...
                        <span>Verified active session ({getAuthedUserEmail()}). Saves directly to your dashboard.</span>
                      </p>
                    ) : (
                      <p className="text-[11px] text-zinc-500 mt-1.5 flex items-center gap-1.5">
                        <span className="text-blue-600 font-bold">🔒</span>
                        <span>A secure 1-click verification link will be sent to verify email ownership before granting dashboard access.</span>
                      </p>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-3 rounded-lg bg-[#0E2342] hover:bg-[#16345E] text-white font-semibold text-xs sm:text-sm shadow-sm transition-all cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {isSavingTrip ? (
                      <>
